...
require('connect.php');
  ...
  //ตรวจสอบว่ารูปแบบการเขียนอีเมลถูกต้อง
    ...
    if(mysqli_num_rows( $call_back_query_check_email_account) > 0) {
        die(header('location: from-register.php')); //อีเมลซ้ำ
    }else{
        $length = random_int(16 , 32);
        $salt_account = bin2hex(random_bytes($length)); //สร้างค่าเกลือ
        $password_account1 = $password_account1 . $salt_account ; //เอารหัสต่อกับค่าเกลือ
        //เปลี่ยนรหัสเป็นตัวอักษรแบบrandom
        ...
        $query_create_account = "INSERT INTO account (id_account, username_account, email_account, password_account, salt_account, ... ) VALUES (...)" ;
        ...
        if ($call_back_create_account){
            ...
            die(header('location : from-register')); //สร้างบัญชีไม่ได้
        }
    }

  }else{
    die(header('location : from-register.php'));
  }
